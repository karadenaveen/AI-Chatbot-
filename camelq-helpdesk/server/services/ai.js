...
import { getLiveRelevantChunks } from './live_retrieval.js';
...
export async function generateAssistantReply({ sessionId, message, history }) {
	...
	const liveMode = Boolean(process.env.LIVE_SITE_BASE_URL);
	const retrieved = liveMode ? (await getLiveRelevantChunks(message, { topK: 6 })) : (await retrieveRelevantChunks(message, { topK: 6 }));
	...
	if (siteContext) {
		messages.push({ role: 'system', content: `CamelQ Site Context (live=${liveMode}):\n\n${siteContext}` });
	}
